...
export default function ProductGraph({ id, onBack, isEmbedded = false }: ProductGraphProps) {
  ...
  return (
    <div className="space-y-4">
      {!isEmbedded && (
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">图谱详情</h1>
            <span className="text-sm text-slate-500 dark:text-slate-400">ID：{id}</span>
          </div>
          <button
            onClick={fetchGraph}
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 dark:border-slate-700 dark:text-slate-200 dark:hover:border-slate-600"
      ...
      {error && (
        <div className="rounded-2xl border border-red-200 bg-red-50/70 p-5 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-200">
          {error}
        </div>
      )}
